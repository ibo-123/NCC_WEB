...
export default function AchievementDetailPage({
  ...
  const achievement = achievements.find(
    (a) => (a._id || a.id) === resolvedParams.id,
  );
    ...
    <div className="space-y-6">
          ...
          <CardContent>
            <div className="text-3xl font-bold">
              {achievement.awardedTo && Array.isArray(achievement.awardedTo)
                ? achievement.awardedTo.length
                : 0}
            </div>
            <p className="text-sm text-slate-600 dark:text-slate-400">
              people have earned this
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">
              ...
              Array.isArray(achievement.awardedTo) &&
              achievement.awardedTo.length <= 10
                ? "Rare"
                : "Common"}
            </div>
            <p className="text-sm text-slate-600 dark:text-slate-400">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Full information about this achievement
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {achievement.awardedTo &&
            Array.isArray(achievement.awardedTo) &&
            achievement.awardedTo.length > 0 && (
              <div>
                ...
                <div className="space-y-2">
                  {achievement.awardedTo
                    ...
                    .map((recipient: any, idx: number) => (
                      <div
                        key={idx}
                        className="text-sm text-slate-700 dark:text-slate-300 p-2 bg-slate-100 dark:bg-slate-800 rounded"
                      >

    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-600 dark:text-slate-400 mt-2">
          Discover and register for upcoming NCC events
        </p>
      </div>
        ...
        <Card>
          <CardContent className="pt-12 text-center">
            ...
            <p className="text-slate-600 dark:text-slate-400">
              {searchTerm ? 'No events match your search' : 'No events available yet'}
            </p>
          </CardContent>
        </Card>
      ) : (
          ...
          {filteredEvents.map((event) => (
            <Card key={event._id || event.id} className="flex flex-col">
              ...
              <CardContent className="flex-1 flex flex-col justify-between">
                ...
                <Link href={`/events/${event._id || event.id}`} className="block">
                ...
                <Button
                  ...
                  className="w-full"
                >
                  {registeringId === (event._id || event.id) ? 'Registering...' : 'Register Now'}
                </Button>
              </CardContent>
            </Card>
          ))}
